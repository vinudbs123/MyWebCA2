...
app.config['DATABASE_URL'] =f'postgres://pebkphcuhaotif:e25d9ba412fe7d5a4edb1a080a73e15d251a640858f536db392d16c46da16b2b@ec2-34-242-199-141.eu-west-1.compute.amazonaws.com:5432/deorjvekdv4daj'
...
def buy():
    ...
    if session:
        ...
        # Select all products for home page view
        products = db.execute("SELECT * FROM products ")
        productsLen = len(products)
...
def filter():
    if request.args.get('sale'):
        ...
        products = db.execute("SELECT * FROM products WHERE onSale = :query ", query=query)
    if request.args.get('id'):
        query = int(request.args.get('id'))
        products = db.execute("SELECT * FROM products WHERE id = :query ", query=query)
    if request.args.get('kind'):
        query = request.args.get('kind')
        products = db.execute("SELECT * FROM products WHERE kind = :query ", query=query)
...
def history():
    ...
    shopLen = len(shoppingCart)
    totItems, total, display = 0, 0, 0
    # Retrieve all products ever bought by current user
